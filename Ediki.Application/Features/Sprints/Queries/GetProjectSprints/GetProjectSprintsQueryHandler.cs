...
public class GetProjectSprintsQueryHandler(ISprintRepository sprintRepository) : IRequestHandler<GetProjectSprintsQuery, Result<IEnumerable<SprintDto>>>
{
    public async System.Threading.Tasks.Task<Result<IEnumerable<SprintDto>>> Handle(GetProjectSprintsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return Result<IEnumerable<SprintDto>>.Success(sprintDtos);
        }
        catch (Exception ex)
        {
            return Result<IEnumerable<SprintDto>>.Failure(ex.Message);
        }
    }
} 